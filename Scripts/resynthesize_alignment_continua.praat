...
form Resynthesize Continuum
	...
	comment Hz values for Pitch Accent
	positive lowHz 80
	positive highHz 120
	comment Original PA values, for calculating L+ and T% targets
	...
	real lowErb -0.25
	real highErb 2
	comment Alignment of Pitch accent & step % (as decimal % of first syllable duration)
	positive paAlignment 1.0
	# comment Alignment step % (shift PA by this % and L+ trails H* by this %)
	positive lowAlignmentPct 0.10
...
alignmentVals# = alignmentVals# * lowAlignmentPct
	...
	for pai from 1 to size (pavals#)
		...
		for bti from 1 to size (btvals#)
  		...
  		for ali from 1 to size (alignmentVals#)
  			...
  			#        origLowHz  btval
  			#             *
  			# *=alignment changes on each iteration
  			...
  			Save as text file: ptDir$ + objname$ + "_" + string$(pai) + "_" + string$(bti) + string$(ali) + ".PitchTier"
  			...
  			selectObject: "TextGrid " + objname$
  			Save as text file: tgToDir$ + objname$ + "_" + string$(pai) + "_" + string$(bti) + "_" + string$(ali) + ".TextGrid"
			endfor
		endfor
	endfor
selectObject: "Sound " + objname$
...
writeFileLine: "resynthesis_parameters.csv", "index, pa_val, bt_val"
for i from 1 to size (pavals#)
	appendFileLine: "resynthesis_parameters.csv", i, ", ", pavals# [i], ", ", btvals# [i]
endfor
