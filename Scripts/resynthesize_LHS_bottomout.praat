...
form Resynthesize Continuum
	...
	text bezierFile exp2bezierpoints.csv
	...
	real lowErb -0.25
	real highErb 2
	comment Percent of post-tonic-syllable duration for L- target
...
for stepi from 1 to size (alignmentSteps#)
	...
	for row from (1+nBezierPoints*alignmentSteps#[stepi]) to (nBezierPoints*alignmentPoints#[stepi])
		selectObject: "Table exp2bezierpoints"
	...
	for bti from 1 to size (btvals#)
		...
		if btvals#[bti] < freqVals#[nBezierPoints]
				Add point: lowPhraseTime, btvals#[bti]
		endif
